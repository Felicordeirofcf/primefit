...
app = FastAPI(
    title="PrimeFit API",
    description="Sistema de autenticação e gerenciamento com Supabase e outras funcionalidades.",
    version="1.0.0",
    docs_url="/docs", # URL para a documentação Swagger UI
...
# Adicione a URL do seu frontend em produção e as URLs de desenvolvimento.
origins = [
    "https://www.primefit.com.br",  # Seu frontend em produção
    "http://localhost:5173",       # Sua origem de desenvolvimento atual (Vite)
    "http://localhost:3000",       # Exemplo comum para Create React App
...
app.add_middleware(
    ...
    allow_credentials=True, # Permite o envio de cookies e tokens de autenticação
...
# Importação das rotas dos diferentes módulos da sua API
# Certifique-se de que os caminhos de importação estão corretos conforme a estrutura do seu projeto
from src.api.endpoints import auth, users, trainings, assessment as assessments, payments, content
# Renomeado 'assessments' para 'assessment_router' na importação para evitar conflito se o módulo se chamar 'assessments'
# Se o seu módulo de avaliações se chama 'assessment.py', a importação original estava correta:
# from src.api.endpoints.assessment import router as assessment_router

# Registro de todas as rotas da aplicação com seus respectivos prefixos e tags
...
app.include_router(assessments.router, prefix="/assessments", tags=["Avaliações"]) # Usando assessments.router diretamente
...
async def root():
    ...
    """
    return {"message": "Bem-vindo à API do PrimeFit! Acesse /docs para a documentação interativa."}

# Bloco para rodar a aplicação com Uvicorn (geralmente para desenvolvimento)
if __name__ == "__main__":
    import uvicorn
    # host="0.0.0.0" faz o servidor ser acessível na rede local
    # reload=True reinicia o servidor automaticamente após mudanças no código (apenas para desenvolvimento)
    uvicorn.run("main:app", host="0.0.0.0", port=int(os.getenv("PORT", 8000)), reload=True)

# ==============================================================================
# PARTE 2: Seu arquivo de router de assessments (ex: src/api/endpoints/assessment.py) revisado.
# (Esta parte permanece como na versão anterior do Canvas)
# ==============================================================================
